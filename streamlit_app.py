...
def enviar_para_assinafy(nome_cliente, email_cliente, pdf_bytes, nome_arquivo):
    ...
    payload = {
        ...
        "signers": [{"name": nome_cliente, "email": email_cliente, "action": "SIGN"}]
    }
    try:
...
if st.session_state.aba_selecionada == "📂 Servidores Já Cadastrados":
    ...
    st.markdown("""
        <div class="box-instrucoes">
            ...
            • <b>Como assinar:</b> Após gerar os documentos, baixe os arquivos para conferência ou utilize o link de assinatura digital gerado para concluir pelo Assinafy.
        </div>
    """, unsafe_allow_html=True)
    ...
    if btn_salvar:
        if not nome.strip():
            ...
            # Tenta enviar para o Assinafy e gerar o link de assinatura digital
    ...
    if tem_documentos:
        ...
        with st.container(border=True):
            ...
            # Botão dinâmico de Assinatura Digital caso o Assinafy retorne o link
